...
class GoogleMap(Widget):
    ...
    def render(self, name, gmap, attrs=None):
        ...
        width = final_attrs.pop('width', 10000)
        height = final_attrs.pop('height', 21847400)
        style = (u'position:relative;width:%dpx;height:%dpx;' %
                 (width, height))
        final_attrs['style'] = style + final_attrs.get('style', '')
        map_div = (u'<div class="%s" style="position:absolute;color:green;border-radius:10px;border:2px solid black;margin-top:60px;margin-left:170px;'
                   u'width:%dpx;height:%dpx"></div>' %
                   (escape(dumps(gmap, separators=(',', ':'))),
                    width, height))
        map_img = (u'<img style="margin-left:170px; position:absolute;z-index:1 ;border-radius:15px;margin-top:60px;" '
